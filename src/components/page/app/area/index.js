...
class AppArea extends Component {
  ...
  toGet() {
    ...
    window.$http.get('/v1/administer/area')
  ...
  toCreate() {
    ...
    let self = this;
    window.$http.post('/v1/administer/area', self.state.form)
      .then(function (response) {
        if (!response || response.data.status !== 0) {
          return false;
        }

        window.$message.success("键帽分区信息添加成功");
  ...
  toUpdate() {
    let self = this;
    window.$http.put('/v1/administer/area/' + self.state.basic.form.id, self.state.form)
      .then(function (response) {
        if (!response || response.data.status !== 0) {
          return false;
        }

        window.$message.success("键帽分区信息修改成功");
  ...
  toDelete = (record) => {
    ...
    window.$confirm({
      ...
      cancelText: '取消',
      onOk() {
        window.$http.delete("/v1/administer/area/" + record.aid)
          .then(function (response) {
            if (!response || response.data.status !== 0) {
              return false;
            }
            window.$message.success("键帽分区信息删除成功");
            self.toGet();
          })
      },
    });
          ...
          <Form onSubmit={this.toSubmit}>
            ...
            >
              {getFieldDecorator('name', {
                ...
                rules: [{
                  required: true, message: '分区名称不能为空',
                }, {
                  max: 20, message: '分区名称最多 20 个字'
                }],
              })(
            ...
            >
              {getFieldDecorator('status', {
                ...
                rules: [{
                  required: true, message: '分区状态不能为空!',
                }],
              })(
                <Select>
